<template>
  <div>
    <!-- 所选商品的界面 -->
    <div
      class="goods_shopping"
      v-for="item in old_goods_shopping"
      :key="item.id"
    >
      <!-- 商品的图片 -->
      <div class="goods_img">
      ...
      >
        <!-- 减 1 的按钮 -->
        <button type="button" class="btn btn-light btn-sm" @click="sub">
          -
        </button>
        <!-- 购买的数量 -->
        <span class="number-box">{{ item.num }}</span>
        <!-- 加 1 的按钮 -->
...
<script>
// 导入 bus 请求库
import bus from "../subcomponents/eventBus.js";
export default {
  data() {
    return {
      // 所选商品的数组
