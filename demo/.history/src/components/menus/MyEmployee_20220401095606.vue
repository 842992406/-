...
export default {
  ...
  methods: {
    changeModifyRow(index, rows) {
      ...
      return Promise.resolve(index, rows)
    },
    // 修改该行的用户数据
    async modifyRow(index, rows) {
      const index = await this.changeModifyRow(index)
      const rows = await this.changeModifyRow(rows)
      // this.$options.methods.changeModifyRow()
