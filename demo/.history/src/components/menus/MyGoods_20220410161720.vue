...
export default {
  ...
  methods: {
    ...
    // 添加商品
    addEmployee() {
      this.form.upload = this.uploadData.url
      const name = this.form.name
      const price = this.form.price
      const upload = this.form.upload
      ...
      this.$refs['form'].validate((valid) => {
        ...
        if (valid) {
          this.$http
            .post(
              '/api/goods/addGoods',
              { name: name, price: price, icon: upload },
              {}
            )
            .then(function (res) {
              console.log(res)
          ...
          ;(this.form.name = ''), (this.form.tel = ''), (this.form.upload = '')
        } else {
    ...
    handleSuccess(response) {
      if (response.url) {
        response.url = response.url.replace('uploads', 'http://localhost:3000')
      }
