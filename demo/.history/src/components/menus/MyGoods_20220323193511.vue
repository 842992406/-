<template>
  <div>
    ...
    >
      ...
      <!-- 上传商品图片 -->
      <el-form-item>
        <p class="form-signIn-heading" id="btnUpload" @change="upload">
          上传图片
        </p>
        <input
          type="file"
          ...
          @change="upload"
        />
        <img src="'http://localhost:8080' + item.photos_url" alt="" />
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    addImage() {
      ...
      this.$axios.post("http://127.0.0.1/upload/img", {}).then((res) => {
        if (res.$data.err == 0) {
