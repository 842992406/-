      ...
      this.$axios.get('/api/goods/getOrders', {}).then((res) => {
        ...
        for (var i = 0; i < res.data.length; i++) {
          if (res.data[i].username == this.$store.state.loginName) {
            this.$data.tableData[m].id = res.data[i].id
            this.$data.tableData[m].price = res.data[i].price
            this.$data.tableData[m].name = res.data[i].name
            this.$data.tableData[m].time = res.data[i].time
            m++
          }
        }
