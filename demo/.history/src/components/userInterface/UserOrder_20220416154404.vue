<template>
  <div>
    ...
    >
      ...
      <el-table-column fixed prop="id" label="编号" width="160">
      </el-table-column>
      ...
      this.$axios.get('/api/goods/getOrders', {}).then((res) => {
        ...
        for (var i = 0; i < res.data.length; i++) {
          if (res.data[i].username == this.$store.state.loginName) {
            this.$data.oldTableData[m] = res.data[i]
            m++
          }
        }
        m = 0
        this.$data.tableData = this.$data.oldTableData
        console.log(this.$data.oldTableData)
