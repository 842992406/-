...
router.post('/addUser', (req, res) => {
  ...
  conn.query("SELECT * FROM user_info WHERE user = '" + username + "'", (err, resu, fields) => {
    let uName = res[0]
    if (uName == undefined) {
      // 不存在时插入数据
      conn.query(sql, { username: params.username, password: params.password }, function (err, result) {
        if (err) {
